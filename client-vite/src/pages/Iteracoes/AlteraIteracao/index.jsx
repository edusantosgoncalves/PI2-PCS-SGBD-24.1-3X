...
function AlteraIteracao(props) {
  ...
  const alteraDados = () => {
    ...
    if (diffDias <= 0) {
      setAbreNaoPode(true);
      setMsgAlerts(
        "Data de conclusão igual ou anterior a data de início: " + diffDias
      );
      return;
    }

    props.alterarIteracao(
      props.iteracao.idIteracao,
        ...
        <Dialog open={props.saiItera} onClose={props.sairItera}>
          ...
          <DialogContent sx={{ backgroundColor: "fundoCard" }}>
            <TextField
              ...
              variant="standard"
              defaultValue={
                props.iteracao === undefined ? "" : props.iteracao.nome
              }
            />
            <TextField
              autoFocus
              ...
              variant="standard"
              defaultValue={
                props.iteracao === undefined ? "" : props.iteracao.descricao
              }
            />

            <Container className="containerNome" sx={{ display: "flex" }}>
              <Typography gutterBottom>Data de Início:</Typography>
              <TextField
                id="dtInicioItera"
                variant="standard"
                type="date"
                defaultValue={
                  props.iteracao === undefined
                    ? ""
                    : props.iteracao.dtInicio.slice(0, 10)
                }
              />
            </Container>

            <Container className="containerNome" sx={{ display: "flex" }}>
              <Typography gutterBottom>Data de Conclusão:</Typography>
              <TextField
                id="dtConcItera"
                variant="standard"
                type="date"
                defaultValue={
                  props.iteracao === undefined
                    ? ""
                    : props.iteracao.dtConclusao.slice(0, 10)
                }
              />
            </Container>
          </DialogContent>
          <DialogActions sx={{ backgroundColor: "fundoCard" }}>
            <Button onClick={props.sairItera}>Cancelar</Button>

            <Button onClick={alteraDados}>Alterar Iteracao</Button>
          </DialogActions>
        </Dialog>
      </ThemeProvider>
